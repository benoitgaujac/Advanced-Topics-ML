...
def process_images(data, predictions, npixels):
    ...
    """
    preds_todraw = np.transpose(preds_todraw,[0,2,1]) #shape: [nbtodraw, 28x28, nsamples]
    if npixels!=300:
...
def save_images(images,npixel,name_model):
    DST = create_DST_DIT("inpainting",name_model)
    DIR_NAME = os.path.join(DST,str(npixel))
...
def create_DST_DIT(folder,name_model):
    DIR = folder
...
def in_painting(model_archi,gt_data,cache_data):
    ...
    # get weights path
    DST = create_DST_DIT("models",nn_model)
    print("")
