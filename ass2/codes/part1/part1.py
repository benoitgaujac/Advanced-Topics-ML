...
def create_DST(name_model):
    NAME = "model_" + str(name_model)
    DIR = "../../trained_models/part1"
    sub_path = os.path.join(DIR,NAME)
    if not tf.gfile.Exists(sub_path):
        os.makedirs(sub_path)
    DST = os.path.join(sub_path,NAME)
    return DST
...
def main(model_archi,train_data, train_labels, validation_data, validation_labels, test_data, test_labels, mode_):
    ...
    logits = build_model_part1.model(data_node, name=nn_model,
    ...
    with tf.Session() as sess:
        ...
        if mode_!="test":
            # opening csv file
            csv_path = "../../trained_models/part1/Perf/Training_"
        ...
        # Testing
        csv_path = "../../trained_models/part1/Perf/test_"
